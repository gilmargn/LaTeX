...
\begin{frame}{Drivers}
    \begin{figure}
        \includegraphics[scale=.56]{driver-printer}
    \end{figure}
\end{frame}
...
\begin{frame}{Guia de Boas Práticas para a Contratação do Serviço de Outsourcing de Impressão}
    \begin{figure}
        \includegraphics[scale=.35]{gov-outsourcing-printing}
    \end{figure}
\end{frame}
...
\begin{frame}[allowframebreaks]{Kyocera Print Center}
    ...
    \begin{itemize}
        \item Descobrir e monitorar impressoras KYOCERA
        \item Digitalizar documentos, capturar imagens e extrair texto usando OCR (Reconhecimento Ótico de Caracteres)
        \item Selecionar ou criar modelos com linhas para imprimir
        \item Definir configurações avançadas de impressora para impressoras KYOCERA
...
%    \begin{figure}
%        \includegraphics[scale=0.25]{nvidia-linux.png}
%    \end{figure}
    ...
    \begin{block}{Pré-requisitos}
        \begin{itemize}
            \item CUPS instalado na distribuição Linux
...
%    \framebreak
    ...
    \begin{itemize}
        ...
        \item Na caixa de diálogo Executar, digite \texttt{services.msc} e pressione Enter para abrir Serviços.
        \item Na janela Serviços, role e localize o serviço \textbf{Spooler de impressão (Print Spooler)}.
        \item Clique duas vezes na entrada para abrir sua janela de propriedades.
        \item Na janela de propriedades, na guia \textbf{Geral}, vá para a seção \textbf{Status do serviço} e clique no botão \textbf{Iniciar} para habilitar o serviço.
        \item Para desabilitar este serviço específico, clique no botão \textbf{Parar}.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Na caixa de diálogo Executar, digite \texttt{cmd} e pressione \texttt{CTRL + SHIFT + ENTER} para abrir o Prompt de Comando em modo administrador.
...
\begin{frame}[allowframebreaks]{Mac OS}
    ...
    \begin{block}{Gutenprint}
        Gutenprint (anteriormente chamado de Gimp-Print) é um pacote de drivers para impressões de alta qualidade para Mac OS X, Darwin, Linux, 
        BSD, Solaris, IRIX, e outros sistemas operacionais semelhantes ao UNIX. Em muitos casos, esses drivers rivalizam ou superam os OEM em qualidade e funcionalidade.
        A meta é produzir resultados com qualidade para todos os dispositivos, mesmo aqueles que já são suportados nativamente.
    \end{block}
    ...
    \begin{figure}
        \includegraphics[scale=0.36]{airprint}
    \end{figure}
...
Os softwares e o hardware evoluiu e popularizou bastante, dessa forma houve uma evolução da impressão.
    \begin{figure}
        \includegraphics[scale=.24]{timeline}
    \end{figure}

    \framebreak

    \begin{figure}
        \includegraphics[scale=0.22]{cloud}
    \end{figure}
\end{frame}

\begin{frame}{Forks}
    \begin{figure}
        \includegraphics[scale=0.23]{fork}
    \end{figure}
\end{frame}
...
\begin{frame}{CUPS compatibilidade}
    ...
    \begin{itemize}
        \item AirPrint\texttrademark{} e IPP Everywhere\texttrademark{} printers
        \item Impressoras de rede e locais (USB) com Aplicações de Impressora
        \item Impressoras de rede e locais (USB) com drivers de impressora baseados em PPD (legado)
    \end{itemize}
\end{frame}
...
\begin{frame}{Interface web}
    \begin{figure}
        \includegraphics[scale=0.35]{cups-web}
    \end{figure}
\end{frame}

\begin{frame}{Gerenciamento de serviços}
		\begin{figure}
			\includegraphics[scale=0.5]{System-Ctl-CUPS}
		\end{figure}
\end{frame}
